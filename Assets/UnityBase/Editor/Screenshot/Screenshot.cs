...
namespace UnityBase.ScreenshotPro
{
	[InitializeOnLoad]
	public class Screenshot : EditorWindow
	{
		...
		{
			var type = EditorUserBuildSettings.activeBuildTarget switch
			{
				BuildTarget.Android => GameViewSizeGroupType.Android,
				BuildTarget.iOS => GameViewSizeGroupType.iOS,
				_ => throw new Exception($"Target platform must be Android or iOS, it is {EditorUserBuildSettings.activeBuildTarget}")
			};

			var size = GameViewUtils.FindSize(type, w, h);
			
			if (size < 0)
			{
				GameViewUtils.AddCustomSize(GameViewUtils.GameViewSizeType.FixedResolution, type, w, h, $"SS_{w}x{h}");
				Debug.Log("Add size " + $"SS_{w}x{h}");
				EditorApplication.Step();
				size = GameViewUtils.FindSize(type, w, h);
			}

			if (size < 0) throw new Exception("Cannot find size (1)");
